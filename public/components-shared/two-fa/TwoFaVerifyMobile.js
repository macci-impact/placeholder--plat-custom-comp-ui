class TwoFaVerifyMobile extends HTMLElement {
  constructor() {
    super();
      ...
      errorMessage.length ?
        `<div class="error-message">
            ...
            <div class="resend-code-holder mt-24">
                ...
                <button id="resend-code-btn" class="btn-span">Resend code</button>
            </div>
          </div>
        </div>
  ...
  async _sendVerification() {
    ...
    try {
      const res = await fetch("/signup/numberValidation/sendMobileValidationPin.json", {
        method: "POST",
        body: JSON.stringify({country, email, number, contactMethod: 'sms'}),
        headers: {
          "content-type": "application/json",
          uitk_csrf: this._uitkCsrf || "",
          "Is-Ajax-Request": "true",
      ...
      if (json.VerificationChannel === "sms") {
...
if (!customElements.get("two-fa-verify-mobile")) {
  customElements.define("two-fa-verify-mobile", TwoFaVerifyMobile);
}
export {TwoFaVerifyMobile};
